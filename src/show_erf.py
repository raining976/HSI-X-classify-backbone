...
parser.add_argument('--source', default='ERF/EyeNet_aug.npy', type=str, help='path to the contribution score matrix (.npy file)')
parser.add_argument('--heatmap_save', default='ERF/EyeNet_aug.png', type=str, help='where to save the heatmap')
args = parser.parse_args()

import numpy as np
#RdYlGn
def heatmap(data, camp='viridis', figsize=(10, 10.75), ax=None, save_path=None):
    ...
    ax = sns.heatmap(data,
                ...
                center=0, annot=False, ax=ax, cbar=False, 
                annot_kws={"size": 24}, fmt='.2f',vmin=mmin,vmax=mmax)
    ...
    # colorbar = plt.colorbar(ax.get_children()[0], cax=cax, orientation='vertical')
    # # 设置颜色条刻度
    # ticks = np.linspace(mmin, mmax, num=6)  # 创建5个刻度
    # colorbar.set_ticks(ticks)  # 设置刻度值
